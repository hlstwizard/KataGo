...
@property (readonly) NSString *modelFile;
...
- (id) init:(NSString *)modelFile :(NSString *)configFile {
  self = [super init];
...
/// Using atomic we probably don't need user defined setter / getter.
//@synthesize inputRequests = _inputRequests;
//
//- (void)setInputRequests:(ThreadSafeQueue<string *> *)newVal {
//  @synchronized(self) {
//      delete _inputRequests;
//      _inputRequests = newVal;
//  }
//}
//
//- (ThreadSafeQueue<string*> *) inputRequests {
//  @synchronized (self) {
...
- (void) addInputRequest: (NSString *)requestBlob {
  @synchronized (self) {
    string* request = new string([requestBlob cStringUsingEncoding:NSUTF8StringEncoding]);
    _inputRequests->waitPush(request);
  }
}
...
- (void) runLoop {
  ...
  NSURL *configUrl = [[NSBundle mainBundle] URLForResource:_configFile withExtension:@"cfg"];
#elif TARGET_OS_OSX
  NSURL *configUrl = [NSURL fileURLWithPath: [NSString stringWithFormat: @"%@.cfg", configFile]];
#endif
  const std::string _modelfile = std::string([_modelFile cStringUsingEncoding:NSUTF8StringEncoding]);
  
  Rand seedRand;
  ConfigParser cfg;
  cfg.initialize([configUrl.path cStringUsingEncoding:NSUTF8StringEncoding]);
  
  cfg.applyAlias("numSearchThreadsPerAnalysisThread", "numSearchThreads");
  
  bool quitWithoutWaiting = true;
  ...
  if(!logToStderr) {
    std::cerr << Version::getKataGoVersionForHelp() << std::endl;
  }
  ...
  NNEvaluator* nnEval;
  {
    ...
    nnEval = Setup::initializeNNEvaluator(_modelfile,_modelfile,expectedSha256,cfg,logger,seedRand,maxConcurrentEvals,expectedConcurrentEvals,
  ...
  cfg.warnUnusedKeys(std::cerr,&logger);
  ...
  auto requestLoop = [&]() {
    ...
    while(_inputRequests->waitPop(pline)) {
